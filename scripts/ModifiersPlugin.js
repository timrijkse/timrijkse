  ...
  var _cssRatioSetter = function(pt, cssp, mod) {
      ...
      pt.setRatio = function(v) {
        ...
        if (v === 1 && (tween._time === tween._duration || tween._time === 0)) {
          ...
          v ||
          !(tween._time === tween._duration || tween._time === 0) ||
          tween._rawPrevTime === -0.000001
        ) {
          val = pt.c * v + pt.s
          if (pt.r) {
            val = Math.round(val)
    ...
    _modCSS = function(lookup, cssp) {
      var pt = cssp._firstPT,
        hasBezier =
          lookup.rotation &&
          cssp._overwriteProps.join('').indexOf('bezier') !== -1 //when a Bezier tween is applying autoRotation, it's a very special case we need to handle differently.
      while (pt) {
        if (typeof lookup[pt.p] === 'function') {
          ...
          hasBezier &&
          pt.n === 'bezier' &&
          pt.plugin._overwriteProps.join('').indexOf('rotation') !== -1
        ) {
